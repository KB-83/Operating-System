...
-----------------------------  KB  $first task first part$--------------------------
  ...
  function InitFirstProcess ()
    ...
    manager.Init ()
    newThread = manager.GetANewThread()
...
function StartUserProcess (arg: int)
    ...
    executable = fileManager.Open("TestProgram1")
    if executable == null
        FatalError("Unable to open executable file TestProgram1")
    endIf
    
    addrSpace = &pcb.addrSpace
    initPC = executable.LoadExecutable(addrSpace)
    if initPC == -1
        FatalError("initPC error.")
    endIf

    fileManager.Close(executable)
...
-----------------------------  Handle_Sys_Shutdown (task 1 part 2)  ---------------------------------
...
-----------------------------  Handle_Sys_Exec KB ---------------------------------
  ...
  function Handle_Sys_Exec(filename: ptr to array of char) returns int
    ...
    result = currentThread.myProcess.addrSpace.GetStringFromVirtual(
    ...
    executable = fileManager.Open(&strBuffer)
    if executable == null
        return -1
    endIf

    initPC = executable.LoadExecutable(&newAddrSpace)
    if initPC == -1
        fileManager.Close(executable)
        return -1
    endIf
    ...
    BecomeUserThread(initUserStackTop, initPC, (&currentThread.systemStack[SYSTEM_STACK_SIZE - 1]) asInteger)
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      var
        strBuffer: array [MAX_STRING_SIZE] of char
        addr : int
    addr = currentThread.myProcess.addrSpace.GetStringFromVirtual(
        &strBuffer, filename asInteger, MAX_STRING_SIZE)
...
-----------------------------  Handle_Sys_Read  ---------------------------------
  ...
  function Handle_Sys_Read (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
       var
        ...
        addr : int
    addr = currentThread.myProcess.addrSpace.GetStringFromVirtual(
        &strBuffer, buffer asInteger, MAX_STRING_SIZE)
